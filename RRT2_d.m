...
while(~is_end)
    ...
    not_full = find(connected_count~=3);
    next_index = randperm(size(not_full,1),1);
    next_index
    q_near_i = not_full(next_index);
    q_near = V(q_near_i,:);
    ...
    if sum(is_in_V,'all') == 0
        ...
        connected_count(next_index) = connected_count(next_index) +1;
    end
end
